document.addEventListener('DOMContentLoaded', () => {
    async function fetchAndUpdateData() {
        try {
            ...
            const response = await fetch('http://192.168.100.100:1880/api/get-data-auto-5');
    ...
    async function submitData(event) {
        ...
        const formData = new FormData(document.getElementById('dataForm-5'));
        const data = {};
        ...
        try {
            const response = await fetch('http://192.168.100.100:1880/api/submit-data-5', {
                method: 'POST',
                headers: {
