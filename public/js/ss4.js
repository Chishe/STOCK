...
document.getElementById('editForm').addEventListener('submit', async function (event) {
    ...
    const response = await fetch(`/api/edit4/${id}`, {
        method: 'PUT',
...
function updateTable(data) {
    ...
    data.forEach(item => {
        ...
        row.innerHTML = `
            ...
            <td>
                <button class="edit" data-id="${item.id}" data-part-no="${item.id_part_no_4}" data-quantity="${item.quantity}">
