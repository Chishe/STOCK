document.getElementById('insertform').addEventListener('submit', async function (event) {
    ...
    try {
        const updateResponse = await fetch('http://192.168.1.100:1880/update-tank-matching-6', {
            method: 'POST',
...
async function insertRowData(tank_btm, tank_top, header_btm, header_top, core_id, model, corenumber) {
    try {
        ...
        const response = await fetch('http://192.168.1.100:1880/api/insert-data-matching-6', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
